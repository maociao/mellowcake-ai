...
class LLMProvider:
    ...
    async def _call_ollama_native(
        ...
        # PATCH: User has Ollama 0.3.12 which doesn't support schema objects in 'format'.
        # We fallback to 'json' format and inject schema into prompt.
        if schema:
            payload["format"] = "json"
            schema_msg = f"\n\nYou must respond with valid JSON matching this schema:\n{json.dumps(schema, indent=2)}"
        ...
        async with httpx.AsyncClient(timeout=300.0) as client:
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt < max_retries:
                        logger.warning(
                            f"Ollama HTTP error (attempt {attempt + 1}/{max_retries + 1}): {e.response.status_code} - {e.response.text}"
                        )
                        backoff = min(initial_backoff * (2**attempt), max_backoff)
                        await asyncio.sleep(backoff)
                        continue
